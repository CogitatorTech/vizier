# See https://github.com/duckdb/extension-ci-tools/blob/main/scripts/append_extension_metadata.py
...
def main():
    arg_parser = argparse.ArgumentParser(
        description='Append extension metadata to loadable DuckDB extensions')

    arg_parser.add_argument('-l', '--library-file', type=str, help='Path to the raw shared library',
                            required=True)
    arg_parser.add_argument('-n', '--extension-name', type=str, help='Extension name to use',
                            required=True)

    arg_parser.add_argument('-o', '--out-file', type=str, help='Explicit path for the output file',
                            default='')

    # The platform
    arg_parser.add_argument('-p', '--duckdb-platform', type=str,
                            help='The DuckDB platform to encode')
    arg_parser.add_argument('-pf', '--duckdb-platform-file', type=str,
                            help='The file containing the DuckDB platform to encode')

    arg_parser.add_argument('-dv', '--duckdb-version', type=str,
                            help='The DuckDB version to encode, depending on the ABI type '
                                 'this encodes the duckdb version or the C API version',
                            required=True)
    arg_parser.add_argument('-ev', '--extension-version', type=str,
                            help='The Extension version to encode')
    arg_parser.add_argument('-evf', '--extension-version-file', type=str,
                            help='The file containing the Extension version to encode')

    arg_parser.add_argument('--abi-type', type=str,
                            help='The ABI type to encode, set to C_STRUCT by default',
                            default='C_STRUCT')
    ...
    if args.duckdb_platform:
        ...
        raise Exception(
            f"Neither --duckdb-platform nor --duckdb-platform-file found, please specify the platform using either")
    ...
    if args.extension_version:
        ...
        if not EXTENSION_VERSION:
            raise Exception(
                f"Extension version file passed to script is empty: {args.extension_version_file}")
    else:
        raise Exception(
            f"Neither --extension-version nor --extension-version-file found, please specify the extension version using either")
