...
- [ ] Query normalization (strip literals, normalize whitespace)
- [ ] Predicate extraction (filter columns, join columns, group-by columns)
...
### Sort / Clustering Advisor

- [ ] Recommend table rewrites with better row order
- [ ] Detect repeated filter prefixes (e.g., tenant_id + ts)
...
- [x] Unit tests (hash determinism, SQL building)
...
- [ ] CI/CD pipeline updates for vizier
